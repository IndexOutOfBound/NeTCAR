...
class EmotionPredictor:
    ...
    def _emoji_mapper(self, tweet):
        command = 'python3 -m emojificate \"{text}\"'.format(text=tweet)
        returned_text = subprocess.check_output(command, shell=True)
        returned_text = returned_text.decode()
        emojis = re.findall(r'<img.*?>', returned_text)
        if emojis:

            for emoji in emojis:
                matched_emoji = re.match(r'.*title="(.*?)".*', emoji).group(1)
                returned_text = returned_text.replace(emoji, ' ' + matched_emoji + ' ')
        returned_text = returned_text.rstrip()
